...
def cadastro_medico(request):
    ...
    if is_medico(request.user):
        messages.add_message(request, constants.WARNING,'Você já possui login de médico')
...
def abrir_horario(request):
    ...
    if request.method == "GET":
        ...
        if data_formatada <= datetime.now():
            messages.add_message(request, constants.WARNING, 'A data deve ser maior que a data atual.')
            return redirect('/medicos/abrir_horario')

        # Salva o objeto com a data formatada
        horario_abrir = DatasAbertas(
            data=data_formatada,  # Usando data_formatada aqui
            user=request.user
        )
...
def finalizar_consulta(request, id_consulta):
    ...
    if request.user != consulta.data_aberta.user:
        messages.add_message(request, constants.ERROR, 'Essa consulta pertence a outro usuário')
...
def add_documento(request, id_consulta):
    ...
    if not documento:
        messages.add_message(request, constants.ERROR, 'É necessário preencher o campo documento.')
        return redirect(f'/medicos/consulta_area_medico/{id_consulta}') 
    ...
    messages.add_message(request, constants.SUCCESS, 'Documento inserido com sucesso.')
